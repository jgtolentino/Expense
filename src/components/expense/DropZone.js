...
var DropZone = function (_a) {
    var getRootProps = _a.getRootProps, getInputProps = _a.getInputProps, isDragActive = _a.isDragActive, isProcessing = _a.isProcessing;
    return (<div {...getRootProps()} className={"border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition-colors\n        ".concat(isDragActive ? 'border-indigo-500 bg-indigo-50' : 'border-gray-300 hover:border-indigo-400', "\n        ").concat(isProcessing ? 'opacity-50 cursor-not-allowed' : '')}>
      <input {...getInputProps()}/>
      {!isProcessing && (<div className="space-y-2">
          ...
          <div className="text-sm text-gray-600">
            {isDragActive ? (<p>Drop the receipt here...</p>) : (<p>
